...
const ADMIN_NUMBERS = [
  "3133221347" // Clean 10 digits (Bina leading 0 ya +92 ke)
];
...
function DashboardPage() {
  ...
  // Strict Phone Match (Bina kisi Format ke Chakkar ke)
  const rawPhone = (profile?.phone || "").replace(/\D/g, ""); // Sirf numbers bachenge
  const isAdminUser = ADMIN_NUMBERS.some((num) => rawPhone.endsWith(num)) || Boolean(profile?.is_admin);
  ...
  return (
    ...
    >
      <div className="space-y-6">
        {/* SIRF AAPKE PHONE NUMBER KO ADMIN BUTTON DIKHAYEGA */}
        {isAdminUser && (
