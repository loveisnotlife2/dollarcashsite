...
// AUTHORIZED ADMIN NUMBERS (Har Format Covered)
...
function DashboardPage() {
  ...
  // Simplified Admin Check (Database Role OR Phone Match)
  const userPhone = (profile?.phone || "").replace(/\D/g, ""); // Strips spaces & symbols
  const isPhoneAdmin = AUTHORIZED_ADMIN_NUMBERS.some((num) => userPhone.endsWith(num.replace(/\D/g, "")));
  const isAdminUser = Boolean(profile?.is_admin) || isPhoneAdmin;
  ...
  return (
    ...
    >
      <div className="space-y-6">
        {/* SIRF AUTHORIZED ADMIN KO BUTTON SHOW HOGA */}
        {isAdminUser && (
